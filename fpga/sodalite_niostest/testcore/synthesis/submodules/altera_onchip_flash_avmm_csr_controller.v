...
module altera_onchip_flash_avmm_csr_controller (
	...
	reg [19:0] csr_page_erase_addr_reg;
	reg [2:0] csr_sector_erase_addr_reg;
	reg [4:0] csr_wp_mode;
	...
	wire is_idle;
	wire is_erase_busy;
	wire valid_csr_page_erase_addr;
	...
	assign csr_page_erase_addr = avmm_writedata[19:0];
	assign csr_sector_erase_addr = avmm_writedata[22:20];
	assign valid_csr_page_erase_addr = (csr_page_erase_addr != {(20){1'b1}});
	assign valid_csr_sector_erase_addr = (csr_sector_erase_addr != {(3){1'b1}});
	assign valid_csr_write = (avmm_write & avmm_addr);		
	assign csr_control_signal = { csr_erase_state, {(2){1'b1}}, csr_wp_mode, csr_sector_erase_addr_reg, csr_page_erase_addr_reg };
	...
	initial begin
		csr_page_erase_addr_reg = {(20){1'b1}};
		csr_sector_erase_addr_reg = {(3){1'b1}};
		csr_wp_mode = {(5){1'b1}};
	...
	always @ (posedge clock) begin
		...
		if (~reset_n_w) begin
			...
			if (valid_csr_write) begin
				...
				if (is_idle) begin 
					csr_page_erase_addr_reg <= avmm_writedata[19:0]; 
					csr_sector_erase_addr_reg <= avmm_writedata[22:20]; 
				end
			end
			...
			case (csr_erase_state)
				ERASE_ST_IDLE:
					if (is_idle && valid_csr_write && (valid_csr_page_erase_addr || valid_csr_sector_erase_addr)) begin 
